...
object ThreadUnsafeClient {
  def fromClientInfo(clientInfo: ClientInfo): ThreadUnsafeClient = {
    ...
    Seq("USD" -> clientInfo.usdBalance, "A" -> clientInfo.aBalance, "B" -> clientInfo.bBalance, "C" -> clientInfo.cBalance,
      "D" -> clientInfo.dBalance).foreach(balances += _)
    new ThreadUnsafeClient(clientInfo.id, balances)
  }
}

class ThreadUnsafeClient(val id: String, balances: mutable.AnyRefMap[String, Long]) extends Client[Long] {
