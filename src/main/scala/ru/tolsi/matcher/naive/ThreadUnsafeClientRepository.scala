...
private[naive] object ThreadUnsafeClientRepository {
  def apply(clients: Seq[Client[Long]]): ThreadUnsafeClientRepository = new ThreadUnsafeClientRepository(clients.map(c => c.id -> c).toMap)
}
private[naive] class ThreadUnsafeClientRepository(private[naive] val map: Map[String, Client[Long]] = Map.empty) extends ClientRepository[Long] {
